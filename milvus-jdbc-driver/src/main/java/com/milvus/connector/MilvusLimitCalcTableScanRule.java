...
@Value.Enclosing
public class MilvusLimitCalcTableScanRule extends RelRule<MilvusLimitCalcTableScanRule.Config> {
    protected MilvusLimitCalcTableScanRule(Config config) {
        super(config);
    }

    @Override
    public void onMatch(RelOptRuleCall relOptRuleCall) {
        System.out.println("hit MilvusLimitCalcTableScanRule");
    ...
    public interface Config extends RelRule.Config {
        ...
        Config DEFAULT = ImmutableMilvusLimitCalcTableScanRule.Config.builder().build()
                .withOperandSupplier(sort -> sort.operand(EnumerableLimit.class)
                        .inputs(scan -> scan.operand(RelNode.class)
                                .inputs(calc -> calc.operand(RelNode.class)
                                        .inputs(table -> table.operand(RelNode.class)
                                                .predicate(rel -> hasMilvusTableScanAncestor(rel)).anyInputs()))));


        @Override
        default MilvusLimitCalcTableScanRule toRule() {
            return new MilvusLimitCalcTableScanRule(this);
        }
